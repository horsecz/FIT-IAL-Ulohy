...
**/
    tBTNodePtr new = (tBTNodePtr) calloc(sizeof(struct tBTNode), 1);
    ...
    while (temp->LPtr != NULL || temp->RPtr != NULL) {
        if (temp->LPtr != NULL && temp->Cont > Content)
            temp = temp->LPtr;
        else if (temp->RPtr != NULL && temp->Cont < Content)
            temp = temp->RPtr;
...
**/
    bool fromLeft;
    tStackP* stackP = (tStackP*) calloc(sizeof(tStackP), 1);
    tStackB* stackB = (tStackB*) calloc(sizeof(tStackB), 1);
...
void Leftmost_Dispose (tBTNodePtr ptr, tStackP* stack) {
    while (ptr != NULL) {
        SPushP(stack, ptr);
        ptr = ptr->LPtr;
    }
...
**/
    tStackP* stack = (tStackP*) calloc(sizeof(tStackP), 1);
    SInitP(stack);
    Leftmost_Dispose((*RootPtr), stack);

    while (!SEmptyP(stack)) {
        (*RootPtr) = STopPopP(stack);
        if ((*RootPtr)->RPtr != NULL)
            Leftmost_Dispose((*RootPtr)->RPtr, stack);
