...
**/
    if ((*RootPtr) != NULL) {
        if ((*RootPtr)->Key > K) {//left
            BSTInsert((&(*RootPtr)->LPtr), K, Content);
        } else if ((*RootPtr)->Key < K) {//right
        ...
        (*RootPtr) = (tBSTNodePtr) calloc(sizeof(struct tBSTNode), 1);
        (*RootPtr)->LPtr = NULL;
        (*RootPtr)->RPtr = NULL;
        (*RootPtr)->BSTNodeCont = Content;
        (*RootPtr)->Key = K;
        ...
        if ((*RootPtr)->Key > K) {
            ...
            BSTDelete(&(*RootPtr)->RPtr, K);
             ...
             if ((*RootPtr)->LPtr == NULL && (*RootPtr)->RPtr == NULL) {
                 ...
                 tBSTNodePtr LPtr = (*RootPtr)->LPtr->LPtr;
                 ReplaceByRightmost((*RootPtr), &(*RootPtr)->LPtr);
                 if ((*RootPtr)->LPtr == NULL)
                    (*RootPtr)->LPtr = LPtr;

             } else {
                 tBSTNodePtr workLPtr = (*RootPtr)->LPtr;
                 tBSTNodePtr workRPtr = (*RootPtr)->RPtr;
                 tBSTNodePtr newRoot = NULL;
